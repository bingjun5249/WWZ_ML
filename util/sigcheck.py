...
hbkg = plt.hist(df_bkg['prediction'], histtype='step', weights=df_bkg['weight']*5/SF, bins=50,linewidth=3, color='crimson', label='Background')
hsig = plt.hist(df_sig['prediction'], histtype='step', weights=df_sig['weight']*5 * lumi * xsex/genevt, bins=50,linewidth=3, color='royalblue', label='Signal')
...
plt.ylim(0.1, 1500)
plt.text(0.3, 300, '(combined channel)', fontsize=20)
plt.legend(fontsize=14, loc='upper left')
plt.yscale('log')
plt.savefig("combined_Nor_DNN_score.png")
#plt.show()
plt.close()
...
Score = list([round(i*0.01, 1) for i in range(0,100)])
...
plt.title("L=3000 $fb^{-1}$ (14 TeV)", fontsize=13, loc='right')
plt.plot(list([round(i*0.02,2) for i in range(0,50)]),arr_sig,'--',color='red',linewidth=2)# label='combined channel', linewidth=2)
plt.xlabel('DNN score',fontsize=25)
plt.ylabel('Expected Significance',fontsize=25)
#plt.text(0.2, 1, '(combined channel)', fontsize=20)
#plt.legend(prop={'size':14})
plt.grid(which='major', linestyle='-')
plt.minorticks_on()
plt.savefig('combined_sig')
#plt.show()
plt.close()
